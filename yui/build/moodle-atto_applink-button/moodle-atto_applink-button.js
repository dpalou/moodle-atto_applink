YUI.add('moodle-atto_applink-button', function (Y, NAME) {
...
/**
 * @module moodle-atto_applink-button
 */

/**
 * Atto text editor app link plugin. The implementation is based on Moodle's link button.
 *
 * @namespace M.atto_applink
 * @class button
 * @extends M.editor_atto.EditorPlugin
 */

var COMPONENTNAME = 'atto_applink',
    CSS = {
        ADDUSERNAME: 'atto_applink_addusername',
        NEWWINDOW: 'atto_applink_openinnewwindow',
        OPENINAPP: 'atto_applink_openinapp',
        URLINPUT: 'atto_applink_urlentry',
        URLSCHEME: 'atto_applink_urlscheme',
        USERNAME: 'atto_applink_username'
    },
    SELECTORS = {
        ADDUSERNAME: '.' + CSS.ADDUSERNAME,
        OPENINAPP: '.' + CSS.OPENINAPP,
        URLINPUT: '.' + CSS.URLINPUT,
        URLSCHEME: '.' + CSS.URLSCHEME,
    ...
    TEMPLATE = '' +
            '<form class="atto_form">' +
                ...
                '{{else}}' +
                    '<div class="mb-1">' +
                        '<label for="{{elementid}}_atto_applink_urlentry">{{get_string "enterurl" component}}</label>' +
                        '<input class="form-control fullwidth url {{CSS.URLINPUT}}" type="url" ' +
                            'id="{{elementid}}_atto_applink_urlentry" size="32"/>' +
                    '</div>' +
                '{{/if}}' +
                ...
                '<hr></hr>' +
                '<div class="form-check">' +
                    '<input type="checkbox" class="form-check-input {{CSS.OPENINAPP}}" id="{{elementid}}_{{CSS.OPENINAPP}}"/>' +
                    '<label class="form-check-label" for="{{elementid}}_{{CSS.OPENINAPP}}">' +
                        '{{get_string "openinapp" component}}' +
                    '</label>' +
                '</div>' +
                '<div class="mb-1">' +
                    '<label for="{{elementid}}_atto_applink_urlscheme">{{get_string "urlscheme" component}}' +
                        ' {{{helpStrings.urlscheme}}}</label>' +
                    '<input class="form-control fullwidth {{CSS.URLSCHEME}}" type="text"' +
                        ' id="{{elementid}}_atto_applink_urlscheme" size="32"/>' +
                '</div>' +
                '<p></p>' +
                '<div class="form-check">' +
                    '<input type="checkbox" class="form-check-input {{CSS.ADDUSERNAME}}" id="{{elementid}}_{{CSS.ADDUSERNAME}}"/>' +
                    '<label class="form-check-label" for="{{elementid}}_{{CSS.ADDUSERNAME}}">' +
                        '{{get_string "addusername" component}}' +
                    '</label>' +
                '</div>' +
                '<div class="mb-1">' +
                    '<label for="{{elementid}}_atto_applink_username">{{get_string "username" component}}' +
                        '{{{helpStrings.username}}}</label>' +
                    '<input class="form-control fullwidth {{CSS.USERNAME}}" type="text" ' +
                    'id="{{elementid}}_atto_applink_username" size="32"/>' +
                '</div>' +
                '<div class="mdl-align">' +
                    '<br/>' +
...
Y.namespace('M.atto_applink').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
     ...
     * extract the url and attributes from the link and set them in the form.
     *
    ...
    _resolveAnchors: function() {
        ...
        if (anchornodes.length > 0) {
            ...
            openInApp = anchornode.hasAttribute('data-app-link');
            scheme = anchornode.getAttribute('data-app-link');
            addUsername = anchornode.hasAttribute('data-username');
            username = anchornode.getAttribute('data-username');
            ...
            if (!openInApp) {
                this._content.one(SELECTORS.OPENINAPP).removeAttribute('checked');
            } else {
                this._content.one(SELECTORS.OPENINAPP).setAttribute('checked', 'checked');
                this._content.one(SELECTORS.URLSCHEME).set('value', scheme);
            }

            if (!openInApp || !addUsername) {
                this._content.one(SELECTORS.ADDUSERNAME).removeAttribute('checked');
                if (addUsername) {
                    this._content.one(SELECTORS.USERNAME).set('value', username);
                }
            } else {
                this._content.one(SELECTORS.ADDUSERNAME).setAttribute('checked', 'checked');
                this._content.one(SELECTORS.USERNAME).set('value', username);
            }
        }
    },
    ...
    _setLinkOnSelection: function(url) {
        ...
        Y.Array.each(anchornodes, function(anchornode) {
            ...
            // Check open in app attributes.
            target = this._content.one(SELECTORS.OPENINAPP);
            if (target.get('checked')) {
                // Open in app is enabled. Check the scheme to use.
                target = this._content.one(SELECTORS.URLSCHEME).get('value').trim();

                if (target) {
                    anchornode.setAttribute('data-app-link', target);
                } else {
                    anchornode.setAttribute('data-app-link', '');
                }

                // Check if we also need to add the username.
                target = this._content.one(SELECTORS.ADDUSERNAME);
                if (target.get('checked')) {
                    // Add username is enabled. Check the username to use.
                    target = this._content.one(SELECTORS.USERNAME).get('value').trim();

                    if (target) {
                        anchornode.setAttribute('data-username', target);
                    } else {
                        anchornode.setAttribute('data-username', '');
                    }
                } else {
                    anchornode.removeAttribute('data-username');
                }
            } else {
                anchornode.removeAttribute('data-app-link');
                anchornode.removeAttribute('data-username');
            }
        }, this);
    ...
    _getDialogueContent: function() {
        ...
        this._content.one(SELECTORS.OPENINAPP).on('change', this._updateEnabled, this);
        this._content.one(SELECTORS.ADDUSERNAME).on('change', this._updateEnabled, this);
    ...
    /**
     * Update disabled status based on whether app link is enabled or not.
     *
    ...
    _updateEnabled: function() {
        var content = this._content,
            openInApp = content.one(SELECTORS.OPENINAPP).get('checked'),
            addUsername = content.one(SELECTORS.ADDUSERNAME).get('checked');

        content.one(SELECTORS.URLSCHEME).set('disabled', !openInApp);
        content.one(SELECTORS.ADDUSERNAME).set('disabled', !openInApp);
        content.one(SELECTORS.USERNAME).set('disabled', !openInApp || !addUsername);
